...
- Реализован простой эндпоинт статистики `GET /stats/assignments` с агрегатами по пользователям и PR.
- Подготовлен интеграционный сценарий `internal/integration/run_tests.sh` (docker-compose + API suite).
- Линтер настроен через `.golangci.yml` и подключён командой `make lint`.
...
> В репозитории лежит `.env` только с дефолтными значениями (без чувствительных данных) - при необходимости смело меняйте их под своё окружение.

### Команды Makefile

- `make help` - подсказка по доступным целям.
- `make build` - сборка docker-образов без запуска контейнеров.
- `make up` - поднять стек `docker-compose` в фоне.
- `make down` - остановить и убрать контейнеры.
- `make logs` - посмотреть логи сервиса `app`.
- `make test` - выполнить `go test ./...`.
- `make lint` - запустить `golangci-lint run --fix`.
- `make clean` - остановить стек и удалить связанные volumes.
